  ...
  function visit(n, ctx) {
    ...
    switch (n.type) {
      ...
      case 'NewExpression': {
        ...
        // (Single variable, not a Set: the shapes above are disjoint, so at
        // most one argument per call can be a sync callback.)
        let syncArg = null;
        if (n.type === 'NewExpression' && n.callee.type === 'Identifier'
            && n.callee.name === 'Promise' && n.arguments.length > 0) {
          syncArg = n.arguments[0];
        ...
        for (const a of n.arguments) {
          if (!a) continue;
          if (a === syncArg && (a.type === 'FunctionExpression' || a.type === 'ArrowFunctionExpression')) {
            visitFunctionSync(a);
          } else {
            visit(a, 'value');
          }
...
function parseJSBlock(source) {
  ...
  const result = { body: ast.program ? ast.program.body : ast.body, comments, parserMode };
  // Cache the parse of this exact string so preflight (same-run, same
  // reference) reuses it instead of parsing twice (P0-2).
  lastJSParse = { source, result };
  return result;
}

// Same-run cache: preflightJS consults this before parsing (identity by
// reference — split-js.js passes the same `source` const to both stages).
let lastJSParse = null;
function getCachedJSParse(source) {
  return (lastJSParse && lastJSParse.source === source) ? lastJSParse.result : null;
}
...
function buildRecords(source, body, comments) {
  ...
  // Single sweep: statements and comments are both in source order, so each
  // comment is visited O(1) times and gap tests use char codes (no slices).
  const sortedComments = comments.slice().sort((a, b) => a.start - b.start);
  let loPtr = 0; // first comment index with end > prevEnd of current stmt
  let hiPtr = -1; // last comment index with end <= s.start of current stmt
  const isWs = (ch) => ch === ' ' || ch === '\t' || ch === '\n' || ch === '\r';
  for (let i = 0; i < stmts.length; i++) {
    const s = stmts[i];
    const prevEnd = i === 0 ? 0 : stmts[i - 1].end;
    while (loPtr < sortedComments.length && sortedComments[loPtr].end <= prevEnd) loPtr++;
    while (hiPtr + 1 < sortedComments.length && sortedComments[hiPtr + 1].end <= s.start) hiPtr++;
    let leadStart = s.start;
    for (let c = hiPtr; c >= loPtr; c--) {
      const cm = sortedComments[c];
      if (cm.end > leadStart) continue;
      let k = cm.end, ok = true;
      while (k < leadStart) {
        if (!isWs(source[k])) { ok = false; break; }
        k++;
      }
      if (!ok) break; // code between → earlier comments can't match either
      leadStart = cm.start;
    }
    s.start = leadStart;
  }
...
module.exports = {
  analyzeJS, parseJSBlock, getCachedJSParse, buildRecords, collectFreeRefs, declaredNamesOfStatement,
