...
function analyzeRecords(records, opts) {
  ...
  // NOTE: on duplicates, refs resolve to the FIRST declaration — a known
  // approximation (which one is "in effect" needs flow analysis).
  ...
  return { declaredAt, duplicateDeclarations, usageCount, hubNames, clusterEdges, orderEdges };
}
...
function clusterRecords(records, clusterEdges, opts) {
  ...
  for (const cl of clusters) {
    if (cl.length > Math.max(30, Math.ceil(records.length * 0.10))) {
      const edges = buildWeightedEdges(cl, clusterEdges);
  ...
  const chars = (cluster) => cluster.reduce((n, i) => n + (records[i].end - records[i].start), 0);
  const merged = [];
  for (const cl of clusters) {
    if (merged.length && chars(cl) < opts.minChars) {
...
function buildWeightedEdges(nodeIds, clusterEdges) {
...
function orderClusters(clusters, orderEdges) {
  ...
  let sccMerged = 0;
  {
    ...
    if (cyclic.length) {
      ...
      console.log(`⚠ Merged ${cyclic.length} cluster group(s) with circular load dependencies into single files — order is now guaranteed, files are bigger.`);
    }
  }
  ...
  while (order.length < nClusters) {
    ...
    const c = available.find(x => !inOrder[x]);
    if (c === undefined) {
  ...
  if (warnedCycle) console.log('⚠ Dependency cycle survived condensation — fell back to original order (verify output!).');
...
function nameClusters(clustersInOrder, records, usageCount, usedNames, ext) {
  ...
  return clustersInOrder.map(cl => {
    ...
    const base = best ? toKebab(best) : `section-${cl[0]}`;
    let file = base + ext;
    let n = 2;
